...
(function() {
    ...
    function encontrarBotaoNovaPesquisa() {
        const elementos = Array.from(document.querySelectorAll('button, a, [role="button"], div, span'));
        ...
        const candidatos = elementos.filter(el => {
            ...
            if (!txt || !txt.includes('NOVA PESQUISA')) return false;

            const rect = el.getBoundingClientRect();

            if (rect.top < window.innerHeight - 190) return false;
            if (rect.width > 280 || rect.height > 95) return false;
        ...
        candidatos.sort((a, b) => {
            ...
            if (Math.abs(rb.right - ra.right) > 5) {
    ...
    function encontrarFallbackDireita() {
        const elementos = Array.from(document.querySelectorAll('button, a, [role="button"], div, span'));
        ...
        const candidatos = elementos.filter(el => {
            ...
            return (
                txt.includes('PASSAGEIROS') ||
                txt === 'IDA' ||
                txt === 'VOLTA' ||
                txt.includes('NOVA PESQUISA')
            );
        });
    ...
    function posicionarBotaoTabela(btn) {
        const larguraBtn = 150;
        const alturaBtn = 38;
        const espaco = 12;

        btn.style.width = larguraBtn + 'px';
        btn.style.height = alturaBtn + 'px';
        btn.style.minWidth = larguraBtn + 'px';
        btn.style.maxWidth = larguraBtn + 'px';
        btn.style.minHeight = alturaBtn + 'px';
        btn.style.maxHeight = alturaBtn + 'px';

        const alvoNovaPesquisa = encontrarBotaoNovaPesquisa();

        if (alvoNovaPesquisa) {
            const rect = alvoNovaPesquisa.getBoundingClientRect();

            let left = rect.left - larguraBtn - espaco;
            let top = rect.top + ((rect.height - alturaBtn) / 2);
        ...
        const fallback = encontrarFallbackDireita();
    ...
    async function desenharECopiarImagem(dados) {
        ...
        const fonteTrecho = 'bold 28px Arial';
